...
fun preprocess_prog exp =
    case exp of
	LineSegment(x1,y1,x2,y2) => (case (real_close(x1,x2), real_close(y1,y2)) of
					 (true,true) => Point(x1,y1)
				       | (true,false)=>(if y2<y1 then  LineSegment(x2,y2,x1,y1) else exp)
				       | (false,_) => (if x2<x1 then LineSegment(x2,y2,x1,y1)  else exp))
