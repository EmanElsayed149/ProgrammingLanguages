...
class Point < GeometryValue
  ...
  def eval_prog env
    self  #all points evaluates to it self
  end
  def preprocess_prog 
    self  #no preprocessing to do here
  end
  def shift(dx,dy)
    Point.new(@x + dx , @y + dy)  #objects shoukd be immutable 
  end
  def intersect other
    other.intersectPoint self
  end
  def intersectPoint p
    return self if real_close_point(@x,@y,p.x,p.y) #if two point are close then intersection will be same point else there is no intersection
    NoPoints.new
  end
  def intersectLine l
    return self if real_close(@y,l.m * @x + l.b)
    NoPoints.new
  end
  def intersectVerticalLine vline
    return self if real_close(vline.x , @x)
    NoPoints.new
  end
  def intersectWithSegmentAsLineResult seg   #there is intersection(point) with line which is extention segment
...
class Line < GeometryValue
  ...
  def eval_prog env  #all lines evaluates to ut sels
    self      
  end
  def preprocess_prog
    self         #no preprocessing to do here
  ...
  def intersectLine line
    ...
    then
      (if real_close(line.b,@b) #they are same line 
      then self
      else NoPoints.new   #they are parallel
       end)
    else
      x=(line.b - @b) / (@m - line.m)
      y= @m * x + @b
...
class LineSegment < GeometryValue
  ...
  def preprocess_prog
    if real_close(@x1,@x2)
    then (if real_close(@y1,@y2) then Point.new(@x1,@y1)
         else
           (if @y2 < @y1 then LineSegment.new(@x2,@y2,@x1,@y1) else self end)
          end)
    else
      (if @x2 < @x1 then LineSegment.new(@x2,@y2,@x1,@y1) else self end)
    end
  end
  def shift(dx, dy)
